...
public class SnakeGame { // MARK
    ...
    public int move(String direction) {
        ...
        if (foodIndex < food.length && rowHead == food[foodIndex][0] && colHead == food[foodIndex][1]) {
            set.add(body.peekLast()); // because whatever eat food, every move will remove the last peek,
                                      // Thus, here add peekLast() is to keep tail remain so that increase the head
