...
/**
 ...
 If we can, it means that the mid value we pick is too small because we've already tried our best to make sure each part holds as many non-negative numbers as we can but we still have numbers left.
...
public class SplitArrayLargestSum {
    public int splitArray(int[] nums, int m) {
        int maxNum = 0, cumSum = 0;
        for (int num : nums) {
            maxNum = Math.max(maxNum, num);
            cumSum += num;
        }

        int L = maxNum, R = cumSum;
        while (L <= R) {
            int mid = L + (R - L) / 2;
            if (largerThanTarget(nums, mid, m)) {
                L = mid + 1;
            } else {
                R = mid - 1;
            }
    ...
    private boolean largerThanTarget(int[] nums, int target, int m) {
        int count = 1;
        int total = 0;
        for (int num: nums) {
            total += num;
